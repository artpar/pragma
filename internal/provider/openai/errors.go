...
// Sentinel errors for OpenAI API error categories.
var (
	ErrRateLimit       = errors.New("openai: rate limit exceeded")
	ErrOverloaded      = errors.New("openai: service overloaded")
	...
	ErrContextOverflow = errors.New("openai: context window exceeded")
)
...
func classifyHTTPError(statusCode int, body []byte, resp *http.Response) classifiedError {
	...
	default:
		return classifiedError{
			wrapped:   fmt.Errorf("openai: HTTP %d: %s", statusCode, errMsg),
			retryable: false,
			errorType: "unknown",
		}
	}
...
// extractErrorMessage parses the OpenAI JSON error body for a human-readable message.

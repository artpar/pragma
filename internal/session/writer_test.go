...
func TestStore_ListMixedFormats(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("HOME", dir)

	store, _ := NewStore()

	// Save a legacy JSON session
	legacySess := Session{
		Conversation: testConversation(),
		Summary:      "legacy session",
		TurnCount:    1,
	}
	store.Save(legacySess)
	...
	w.WriteMessage(model.Message{
		ID: "jm-1", Role: model.RoleUser,
		Content:   []model.ContentPart{model.TextPart{Text: "jsonl hello"}},
		Timestamp: time.Now(),
	})
	w.WriteMetadata(MetadataData{TurnCount: 1, Summary: "jsonl session", UpdatedAt: time.Now()})
	w.Close()
	...
	if len(list) != 2 {
		t.Fatalf("List = %d, want 2 (legacy + jsonl)", len(list))
	}
	...
	if ids["test-session-001"] != "legacy session" {
		t.Errorf("legacy summary = %q", ids["test-session-001"])
	}
	if ids["jsonl-session"] != "jsonl session" {
		t.Errorf("jsonl summary = %q", ids["jsonl-session"])
	}
}

func TestStore_JSONLPreferredOverJSON(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("HOME", dir)
	...
	// Also create JSONL with same session ID
	header := HeaderData{
	...
	w.WriteMessage(model.Message{
		ID: "jm-1", Role: model.RoleUser,
		Content:   []model.ContentPart{model.TextPart{Text: "jsonl"}},
		Timestamp: time.Now(),
	})
	w.WriteMetadata(MetadataData{Summary: "new jsonl", UpdatedAt: time.Now()})
	w.Close()

	// Load should prefer JSONL
	sess, err := store.Load("test-session-001")
	if err != nil {
		t.Fatal(err)
	}

	// JSONL has 1 message, legacy has 3
	if len(sess.Conversation.Messages) != 1 {
		t.Errorf("Messages = %d, want 1 (JSONL should be preferred)", len(sess.Conversation.Messages))
	}
	...
	if count != 1 {
		t.Errorf("Duplicate entries for same ID: count = %d", count)
	}
}

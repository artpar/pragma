...
func TestNewRuntimeUsesLooplabFSM(t *testing.T) {
	runtime, err := NewRuntime(Definition{
		...
		States: []State{
			{ID: "planner"},
			{ID: "implementer"},
			{ID: StateDone, Terminal: true},
		},
		Transitions: []Transition{
			{Event: EventComplete, From: []string{"planner"}, To: "implementer"},
			{Event: EventComplete, From: []string{"implementer"}, To: StateDone},
		},
	})
	if err != nil {
	...
	if got := runtime.FSM.Current(); got != StateDone {
		t.Fatalf("state after second transition = %q, want %s", got, StateDone)
	}
}
...
func TestNewRuntimeRejectsTerminalTransitionSource(t *testing.T) {
	_, err := NewRuntime(Definition{
		...
		Transitions: []Transition{
			{Event: EventComplete, From: []string{StateDone}, To: "solver"},
		},
	})
	if err == nil {
...
func TestLoadArchitectImplementerProsecutorYAML(t *testing.T) {
	...
	if got := runtime.FSM.Current(); got != StateDone {
		t.Fatalf("final state = %q, want %s", got, StateDone)
	}
}

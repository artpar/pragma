...
const grepDescription = `A powerful search tool built on ripgrep
  ...
  Usage:
  - Use this tool for content search tasks. Avoid shell search commands when this tool can accomplish the task.
  - Supports full regex syntax (e.g., "log.*Error", "function\\s+\\w+")
  - Filter files with glob parameter (e.g., "*.js", "**/*.tsx") or type parameter (e.g., "js", "py", "rust")
  - Output modes: "content" shows matching lines, "files_with_matches" shows only file paths (default), "count" shows match counts
  - Use an available delegation or research capability for open-ended searches requiring many rounds
...
func (t *Tool) CheckPerm(ctx context.Context, input json.RawMessage, checker permission.Checker) permission.CheckResult {
	...
	observe.TraceCtx(ctx, "grep", "Tool.CheckPerm", "return: permission.CheckPath(ctx, checker, \"Grep\", in.Path)")
...
func buildContentResult(lines []string, headLimit, offset int, workDir string) string {
	...
	if out == "" {
		...
		out = "IMPORTANT: No matches were found for this pattern. Do NOT fabricate results. Try: case-insensitive (-i), partial name, different naming convention, or broader pattern."
	}
